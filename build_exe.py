...
"""
build_exe.py
使用 PyInstaller 將 NoxPSG Viewer 打包成獨立的 Windows 可執行檔 (.exe)

使用方式 (在 Windows PowerShell 或 CMD 執行，建議用乾淨 venv)：
  python build_exe.py

推薦輸出：
  - dist/NoxPSGViewer/ 目錄 (one-folder，啟動較快、相依完整，強烈建議)
...
def build():
    print("=== 開始打包 NoxPSG Viewer (PyQt6 + pyqtgraph + noxreader) ===")
    ...
    print("\n" + "="*60)
    print("【打包完成】")
    exe_dir = DIST / "NoxPSGViewer"
    if exe_dir.exists():
        exe = exe_dir / "NoxPSGViewer.exe"
        print(f"可執行檔目錄：{exe_dir}")
        print(f"主執行檔：{exe}")
        print("")
        print("★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★")
        print("重要提醒：")
        print("  • 請「只」執行 dist 資料夾裡的 NoxPSGViewer.exe")
        ...
        print("⚠️  請檢查 dist/ 目錄下是否有 NoxPSGViewer 資料夾。")
    ...
    if exe_dir.exists():
        ...
        note_content = """NoxPSG Viewer - 使用注意事項

【最重要】
• 請只執行這個資料夾裡的 「NoxPSGViewer.exe」
• 不要執行專案根目錄下 build\\ 資料夾裡的任何 .exe
  （build 資料夾是 PyInstaller 建置時的暫存檔，執行它會出現：
   "Failed to load Python DLL '...\\build\\NoxPSGViewer\\_internal\\python312.dll'" 的錯誤）

• 建議建置完後可以直接刪除整個 build 資料夾。

【正確啟動方式】
1. 直接雙擊 NoxPSGViewer.exe
...
【分發給別人】
把整個 NoxPSGViewer 資料夾壓縮成 zip 即可。
